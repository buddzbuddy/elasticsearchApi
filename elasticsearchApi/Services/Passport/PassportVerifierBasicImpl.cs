    ...
    public class PassportVerifierBasicImpl : IPassportVerifierBasic
    {
        public void Verify(modifyPersonPassportDTO passport)
        {
            ...
            if (passportType == null)
                throw new PassportInputErrorException("PassportType", "Тип удостоверяющего документа не указан!");
            ...
            if (isPassport && string.IsNullOrWhiteSpace(series))
                throw new PassportInputErrorException("passportseries", "Серия удостоверяющего документа не указана!");
            v = passport.passportno;
            var no = v != null ? v.ToString().Trim() : string.Empty;
            if (isPassport && string.IsNullOrWhiteSpace(no))
                throw new PassportInputErrorException("passportno", "Номер удостоверяющего документа не указан!");
            var pDate = passport.date_of_issue;
            if (isPassport && pDate == null)
                throw new PassportInputErrorException("date_of_issue", "Дата выдачи удостоверяющего документа не указана!");
            var issueDate = pDate;
            ...
            if (isPassport && string.IsNullOrWhiteSpace(authority))
                throw new PassportInputErrorException("issuing_authority", "Орган выдавший удостоверяющий документ не указан!");
            var familyState = passport.familystate;
            if (isPassport && familyState == null)
                throw new PassportInputErrorException("familystate", "Семейное положение не указано!");
        }
    }
}
